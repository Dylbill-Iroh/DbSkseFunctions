                                ...
                                if (graphData) {
                                    ...
                                    if (data) {
                                        ...
                                        if (stringData) {
											...
											logger::info("size[{}] data[{}] elem0raw[{:016X}] elem1raw[{:016X}] sizeofElem[{}]",
												characterPropertyNames.size(),
												static_cast<const void*>(characterPropertyNames.data()),
												*reinterpret_cast<const std::uint64_t*>(characterPropertyNames.data()),
												*reinterpret_cast<const std::uint64_t*>(characterPropertyNames.data()) ? *(reinterpret_cast<const std::uint64_t*>(characterPropertyNames.data()) + 1) : 0,
												sizeof(RE::hkStringPtr)
											);
											
											if (characterPropertyNames.size() > 0) {
                                                for (auto& name : characterPropertyNames) {
                                                    auto* cname = name.c_str();
													std::size_t len = gfuncs::GetUnsignedCharLength(cname);
													
													if (len > 0){
														CharacterProperties.emplace_back(cname, len);
                                        ...
                                        if (stringData) {
                                            ...
                                            if (attributeNames.size() > 0) {
                                                for (auto& name : attributeNames) {
													...
													if (len > 0){
														attributes.emplace_back(cname, len);
                                                        //logger::info("[{}] ref graph variable[{}]", refName, cname);
													} 
                                                }
                                            }
        ...
        if (manager) {
            ...
            if (manager->variableCache.variableCache.size() > 0) {
                for (uint32_t i = 0; i < manager->variableCache.variableCache.size(); i++) {
					...
					const char* vn = value.variableName.c_str();
					if (gfuncs::GetUnsignedCharLength(vn) == 0) { 
						continue; 
					}
					
					std::string sVariable(vn);
                    if (sVariable != "") {
