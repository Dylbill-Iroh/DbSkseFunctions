...
namespace serialize {
	...
	bool SaveString(uint32_t record, SKSE::SerializationInterface* ssi, std::string s) {
        std::string sRecord = gfuncs::uint32_to_string(record);
		
		const std::uint32_t len = static_cast<std::uint32_t>(s.length());
        if (!ssi->WriteRecordData(&len, sizeof(len))) {
            logger::error("failed to write string length for [{}] record[{}]", s, sRecord);
            return false;
        }
        if (len > 0 && !ssi->WriteRecordData(s.data(), len)) {
            logger::error("failed to write string data for [{}] record[{}]", s, sRecord);
            return false;
        }
		return true;
    }
	
    std::string LoadString(uint32_t record, SKSE::SerializationInterface* ssi) {
        std::string sRecord = gfuncs::uint32_to_string(record);
		
		std::string s = ""; 
		
        std::uint32_t len = 0;
        if (!ssi->ReadRecordData(len)) {
            logger::error("failed to read string length for record[{}]", sRecord);
            return "";
        }
		
        s.resize(len);
        if (len > 0 && ssi->ReadRecordData(s.data(), len) != len) {
            logger::error("failed to read string data for record[{}]", sRecord);
            s.clear();
            return "";
        }
        return s;
