...
namespace UIEvents {
    ...
    struct UiFormData {
        ...
        UiFormData(){
            ...
            stolen = false;
            //logger::trace("{}: default constructor", __func__);
        }
        ...
        UiFormData(RE::TESForm* akForm, int akCount, bool akSelectedFromPlayerInventory, bool akStolen) :
            ...
            stolen(akStolen)
        {
            //logger::trace("{}: specific constuctor", __func__); 
        }
                                        ...
                                        if (gfuncs::IsFormValid(akForm)) {
                                            ...
                                            if (menu->uiMovie->GetVariable(&gfxStolen, "_root.Menu_mc.inventoryLists.itemList.selectedEntry.isStolen")) { //
                                                if (gfxStolen.IsBool()) {
                                                    ...
                                                    //logger::trace("{}: selectedEntry.isStolen = [{}]", __func__, isStolen);
                                                }
                                            }

                                            //logger::trace("{}: owner is [{}]", __func__, gfuncs::GetFormName(owner));
                        ...
                        if (gfuncs::IsFormValid(akForm, false)) {
                            ...
                            if (menu->uiMovie->GetVariable(&gfxCount, "_root.Menu.InventoryLists.panelContainer.itemList.selectedEntry.count")) {
                                if (gfxCount.IsNumber()) {
                                    ...
                                    //logger::trace("{}: selectedEntry.count = [{}]", __func__, count);
                                }
                            }
                        ...
                        if (gfuncs::IsFormValid(akForm, false)) {
                            ...
                            if (magicItem) {
                                ...
                                //if (count <= 0) { // CountObjectsInContainer doesn't work for all forms, for example (iron boots), getting item count from text instead. 
                                    ...
                                    //logger::trace("{}: sText = [{}] count = [{}]", __func__, sText, count);
                                //}
                            }
    ...
    bool ShouldSkipUIItemEvent() {
        if (ui->closingAllMenus) {
            logger::trace("{}: closingAllMenus = true", __func__);
            return true;
        }

        if (!ui->menuSystemVisible) {
            logger::trace("{}: menuSystemVisible = false", __func__);
            return true;
        }
    ...
    struct InventoryMenuHook {
        ...
        static inline void Install() {
            if (!inventoryMenuHookInstalled) {
                ...
                logger::debug("{}: installing inventory menu hook", __func__);
            }
        }
    };
    ...
    struct BarterMenuHook {
        ...
        static inline void Install() {
            if (!barterMenuHookInstalled) {
                ...
                logger::debug("{}: installing barter menu hook", __func__);
            }
        }
    };
    ...
    struct ContainerMenuHook {
        ...
        static inline void Install() {
            if (!containerMenuHookInstalled) {
                ...
                logger::debug("{}: installing Container menu hook", __func__);
            }
        }
    };
    ...
    struct GiftMenuHook {
        ...
        static inline void Install() {
            if (!giftMenuHookInstalled) {
                ...
                logger::debug("{}: installing Gift menu hook", __func__);
            }
        }
    };
    ...
    struct MagicMenuHook {
        ...
        static inline void Install() {
            if (!magicMenuHookInstalled) {
                ...
                logger::debug("{}: installing Magic menu hook", __func__);
            }
        }
    };
    ...
    struct CraftingMenuHook {
        ...
        static inline void Install() {
            if (!craftingMenuHookInstalled) {
                ...
                logger::debug("{}: installing Crafting menu hook", __func__);
            }
        }
    };
    ...
    struct FavoritesMenuHook {
        ...
        static inline void Install() {
            if (!favoritesMenuHookInstalled) {
                ...
                logger::debug("{}: installing Favorites menu hook", __func__);
            }
        }
    };

    void InstallUiEventHook(int index) {
        logger::trace("{}: called. index = {}", __func__, index);
        switch (index) {
        ...
        default:
            logger::trace("{}: installing all hooks", __func__);
            InventoryMenuHook::Install();
            BarterMenuHook::Install();
            ContainerMenuHook::Install();
            GiftMenuHook::Install();
    ...
    bool IsFormRegisteredForUiItemMenuEvent(RE::StaticFunctionTag*, std::string menuName, RE::TESForm* eventReceiver, RE::TESForm* paramFilter, int eventType) {
        if (!gfuncs::IsFormValid(eventReceiver)) {
            logger::warn("{}: eventReceiver doesn't exist or isn't valid", __func__);
            return false;
        }
        ...
        if (handle == NULL) {
            logger::error("{}: couldn't get handle for eventReceiver[{}]", __func__, gfuncs::GetFormName(eventReceiver));
            return false;
        }
        ...
        if (menuName != "") {
            ...
            if (menuNameIndex == -1) {
                logger::error("{}: menuName [{}] not recognized", __func__, menuName);
                return false;
            }
        }
    ...
    bool IsAliasRegisteredForUiItemMenuEvent(RE::StaticFunctionTag*, std::string menuName, RE::BGSBaseAlias* eventReceiver, RE::TESForm* paramFilter, int eventType) {
        ...
        if (handle == NULL) {
            logger::error("{}: couldn't get handle for eventReceiver[{}]", __func__, eventReceiver->aliasName);
            return false;
        }
    ...
    bool IsActiveMagicEffectRegisteredForUiItemMenuEvent(RE::StaticFunctionTag*, std::string menuName, RE::ActiveEffect* eventReceiver, RE::TESForm* paramFilter, int eventType) {
        ...
        if (handle == NULL) {
            logger::error("{}: couldn't get handle for eventReceiver[{}]", __func__, gfuncs::GetFormName(eventReceiver->GetBaseObject()));
            return false;
        }
